...
type Command = {code:number|null; stdout:string; stderr:string; timedOut:boolean;aborted:boolean;outputLimited:boolean};
export type RunnerOptions = {dockerExecutable?:string; dockerArgumentsPrefix?:string[]; image?:string; tempRoot?:string};
export class JavaRunner {
  private readonly docker:string; private readonly dockerArgumentsPrefix:readonly string[]; private readonly image:string; private readonly tempRoot:string;
  ...
  private availabilityController:AbortController|undefined;
  constructor(options:RunnerOptions={}) {this.docker=options.dockerExecutable??'docker';this.dockerArgumentsPrefix=Object.freeze([...(options.dockerArgumentsPrefix??[])]);this.image=options.image??RUNNER_IMAGE;this.tempRoot=options.tempRoot??tmpdir();}
  private command(args:string[], timeout=5000, signal?:AbortSignal, maxBytes=LIMITS.taskProtocolBytes):Promise<Command> {
    return new Promise(resolve=>{
      const child=spawn(this.docker,[...this.dockerArgumentsPrefix,...args],{windowsHide:true,stdio:['ignore','pipe','pipe']});
      const stdout:Buffer[]=[];const stderr:Buffer[]=[];let bytes=0;let timedOut=false;let aborted=false;let outputLimited=false;let finished=false;
      const stopForTimeout=()=>{timedOut=true;child.kill();}; const stopForAbort=()=>{aborted=true;child.kill();}; const timer=setTimeout(stopForTimeout,Math.max(1,timeout));
      signal?.addEventListener('abort',stopForAbort,{once:true}); if(signal?.aborted) stopForAbort();
      ...
      const finish=(code:number|null)=>{if(finished)return;finished=true;clearTimeout(timer);signal?.removeEventListener('abort',stopForAbort);resolve({code,stdout:Buffer.concat(stdout).toString(),stderr:Buffer.concat(stderr).toString(),timedOut,aborted,outputLimited});};
  ...
  private remaining(deadline:number|undefined, cap:number):number {return deadline===undefined?cap:Math.max(0,Math.min(cap,deadline-Date.now()));}
  private stopped(signal:AbortSignal|undefined,deadline:number|undefined):boolean {return signal?.aborted===true||(deadline!==undefined&&Date.now()>=deadline);}
  async getAvailability(options:{signal?:AbortSignal;deadline?:number}={}):Promise<{available:boolean;reason:string|null}> {
    if(this.closed)return {available:false,reason:'RUNNER_CLOSED'};
    if(options.signal!==undefined||options.deadline!==undefined)return this.checkAvailability(options.signal,options.deadline);
    if(!this.availabilityPending) {
      const controller=new AbortController();this.availabilityController=controller;
      this.availabilityPending=this.checkAvailability(controller.signal,Date.now()+LIMITS.totalMs).finally(()=>{this.availabilityPending=undefined;this.availabilityController=undefined;});
    }
    return this.availabilityPending;
  }
  private async checkAvailability(signal?:AbortSignal,deadline?:number):Promise<{available:boolean;reason:string|null}> {
    if(this.availability && Date.now()-this.availability.at<10000)return this.availability.value;
    if(this.stopped(signal,deadline))return {available:false,reason:signal?.aborted?'RUNNER_CHECK_ABORTED':'RUNNER_CHECK_TIMEOUT'};
    const info=await this.command(['info','--format','{{.OSType}}'],this.remaining(deadline,5000),signal);
    if(info.aborted)return {available:false,reason:'RUNNER_CHECK_ABORTED'};
    if(info.timedOut)return {available:false,reason:'DOCKER_CHECK_TIMEOUT'};
    if(info.code!==0)return {available:false,reason:info.stderr.includes('ENOENT')?'DOCKER_CLI_UNAVAILABLE':'DOCKER_DAEMON_UNAVAILABLE'};
    const engineType=info.stdout.trim();
    if(engineType!=='linux')return {available:false,reason:engineType?'DOCKER_ENGINE_NOT_LINUX':'DOCKER_DAEMON_UNAVAILABLE'};
    try{await this.recoverStale(signal,deadline);}catch(error){return {available:false,reason:`STALE_CLEANUP_FAILED: ${String(error)}`};}
    if(this.stopped(signal,deadline))return {available:false,reason:signal?.aborted?'RUNNER_CHECK_ABORTED':'RUNNER_CHECK_TIMEOUT'};
    const name=`jab-probe-${randomUUID()}`;this.containers.add(name);
    const run=await this.command(['run','--pull=never',...this.constraints(name),this.image,'--probe'],this.remaining(deadline,10000),signal);
    const cleanup=await this.remove(name,undefined,Date.now()+5000);
    if(run.aborted)return {available:false,reason:'RUNNER_CHECK_ABORTED'};
    if(run.timedOut)return {available:false,reason:'DOCKER_CHECK_TIMEOUT'};
  ...
  private async remove(name:string,signal?:AbortSignal,deadline:number=Date.now()+5000):Promise<boolean> {
    const remaining=this.remaining(deadline,5000);if(remaining<=0)return false;
    const removed=await this.command(['rm','-f',name],remaining,signal);
    const okay=!removed.aborted&&!removed.timedOut&&(removed.code===0 || removed.stderr.includes('No such container'));
    if(okay)this.containers.delete(name);return okay;
  }
  private async recoverStale(signal?:AbortSignal,deadline?:number):Promise<void> {
    const cutoff=Date.now()-120000;
    const listBudget=this.remaining(deadline,5000);if(listBudget<=0)throw new Error('STALE_CLEANUP_TIMEOUT');
    const listed=await this.command(['ps','-aq','--filter',`label=${CONTAINER_LABEL}`],listBudget,signal);
    if(listed.aborted)throw new Error('STALE_CLEANUP_ABORTED');
    if(listed.timedOut)throw new Error('STALE_CLEANUP_TIMEOUT');
    ...
    for(const id of listed.stdout.trim().split(/\s+/).filter(Boolean)) {
      if(this.stopped(signal,deadline))throw new Error(signal?.aborted?'STALE_CLEANUP_ABORTED':'STALE_CLEANUP_TIMEOUT');
      if(!/^[a-f0-9]{12,64}$/.test(id))continue;
      const inspectBudget=this.remaining(deadline,5000);if(inspectBudget<=0)throw new Error('STALE_CLEANUP_TIMEOUT');
      const inspected=await this.command(['inspect','--format','{{.Created}}',id],inspectBudget,signal);
      if(inspected.aborted||inspected.timedOut)throw new Error(inspected.aborted?'STALE_CLEANUP_ABORTED':'STALE_CLEANUP_TIMEOUT');
      if(inspected.code===0 && Date.parse(inspected.stdout.trim())<cutoff && !await this.remove(id,signal,deadline))throw new Error('Cannot remove stale project container');
    }
  ...
  async execute(raw:RunRequest, options:{requestId?:string;signal?:AbortSignal}={}):Promise<RunResult> {
    const started=Date.now();const deadline=started+LIMITS.totalMs;const request=RunRequestSchema.parse(raw);
    ...
    try {
       result.phase='infrastructure';const availability=await this.getAvailability({signal:controller.signal,deadline});
       if(controller.signal.aborted){result.status='CANCELLED';result.reason='ABORTED';return finish();}
       if(Date.now()>=deadline){result.status='SYSTEM_ERROR';result.reason='RUNNER_CHECK_TIMEOUT';return finish();}
      ...
      const executed=await this.command(['run','--pull=never',...this.constraints(name),'--mount',`type=bind,source=${directory},target=/input,readonly`,this.image],Math.max(1,deadline-Date.now()),controller.signal);
      if(controller.signal.aborted){result.status='CANCELLED';result.reason='ABORTED';return finish();}
      ...
      const cleanupDeadline=Date.now()+5000;let cleaned=true;if(name)cleaned=await this.remove(name,undefined,cleanupDeadline);
  ...
  async close():Promise<void>{
    this.closed=true;this.availabilityController?.abort();for(const task of this.tasks)task.abort();
    const deadline=Date.now()+5000;
    while(this.active&&Date.now()<deadline)await new Promise(resolve=>setTimeout(resolve,20));
    if(this.availabilityPending)await Promise.race([this.availabilityPending.catch(()=>undefined),new Promise(resolve=>setTimeout(resolve,Math.max(1,deadline-Date.now())))]);
    const names=[...this.containers];const removed=await Promise.all(names.map(name=>this.remove(name,undefined,deadline)));
    const failed=names.filter((_,index)=>!removed[index]);
    if(this.active||this.availabilityPending||failed.length)throw new Error(`RUNNER_CLOSE_CLEANUP_FAILED: ${[this.active?'active-task':null,this.availabilityPending?'availability-check':null,...failed].filter(Boolean).join(', ')}`);
  }
}
